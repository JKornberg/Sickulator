...
class Simulation:
    ...
    def new(self):
        ...
        home_coords = [
            ...
            (8, 47), (22, 47), (36, 47), (50, 47),
            (8, 59), (22, 59), (36, 59), (50, 59),
        ...
        for x in range(0, math.ceil(num_agents/self.simulation_settings.family_size)):
            ...
            new_home = home_coords[x]
            self.families.append(Family(self, new_home))
            # removes the possibility of duplicate homes -- NOT SUITABLE FOR num_agents > 20*family size
            home_coords.remove(new_home)

        # puts agents in families; fills families before moving to new ones
        for index in range(0, num_agents):
            family_to_fill = self.families[int(
                index/self.simulation_settings.family_size)]
            family_to_fill.add_agent(
                Agent(self, family_to_fill, family_to_fill.home[0], family_to_fill.home[1]))
    ...
    def make_gui(self):
        ...
        self.timer = pygame_gui.elements.UILabel(relative_rect=pg.Rect(
            (100, 0), (100, 50)), text="Day: 0", container=bottom_bar, manager=self.gui)
        ...
        self.double_speed_button = pygame_gui.elements.UIButton(relative_rect=pg.Rect((300, 0), (50, 50)), text="2x",
                                                                manager=self.gui, container=bottom_bar)
        self.info = pygame_gui.elements.UIPanel(relative_rect=pg.Rect(
            (0, 0), (350, HEIGHT-80)), starting_layer_height=0, manager=self.gui, visible=False)
        ...
        self.description = pygame_gui.elements.UITextBox(html_text="""<body bgcolor='0xFFFFFF'>THE SICKULATOR<br>The Sickulator is an agent-based simulator which visualizes the spread of disease in a small city. Agents begin every day at home with their family where they select a schedule for the day. They then venture out to the city and visit all the buildings their schedule includes. A small subset of agents begin the simulation infected, and we can watch the disease spread over time. The health status of an agent is represented by their color. Green is healthy, red is infected, blue is immune, and dark grey is deceased.</body>""", relative_rect=pg.Rect((0, 0), (300, 400)), container=self.info, manager=self.gui, layer_starting_height=2)
